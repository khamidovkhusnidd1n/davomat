...
import { supabase } from '@/lib/supabase'; // Using the admin app's supabase client
...
bot.hears('📅 Mening davomatim', async (ctx) => {
  ...
  if (!attendances || attendances.length === 0) return ctx.reply("Sizda hali davomat ma'lumotlari yo'q.");
...
bot.hears('📚 Dars mavzulari', async (ctx) => {
  ...
  if (!syllabuses || syllabuses.length === 0) return ctx.reply("Sizning guruhingiz uchun hali dars dasturi kiritilmagan.");
